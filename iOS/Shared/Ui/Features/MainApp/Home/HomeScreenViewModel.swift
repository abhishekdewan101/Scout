...
class HomeScreenViewModel: ObservableObject {
    ...
    private func getComingSoonGames() {
        gameRepository.getListDataForType(type: ListType.comingSoon) { (coreFlow: Kotlinx_coroutines_coreFlow?, error: Error?) in
            FlowExtensionsKt.asCommonFlow(coreFlow!).watch { (object: AnyObject?) in
    ...
    private func getRecentGames() {
        gameRepository.getListDataForType(type: ListType.recent) { (coreFlow: Kotlinx_coroutines_coreFlow?, error: Error?) in
            FlowExtensionsKt.asCommonFlow(coreFlow!).watch { (object: AnyObject?) in
    ...
    private func getHypedGames() {
        gameRepository.getListDataForType(type: ListType.mostHyped) { (coreFlow: Kotlinx_coroutines_coreFlow?, error: Error?) in
            FlowExtensionsKt.asCommonFlow(coreFlow!).watch { (object: AnyObject?) in
    ...
    private func getTopRatedGames() {
        gameRepository.getListDataForType(type: ListType.topRated) { (coreFlow: Kotlinx_coroutines_coreFlow?, error: Error?) in
            FlowExtensionsKt.asCommonFlow(coreFlow!).watch { (object: AnyObject?) in
